<?php
class ControllerExtensionModuleDkaCart extends Controller {
...
private $dkaModuleBuildVersion = "1";
private $dkaModuleDateRelease = "12.10.2016";
private $dkaModuleAddressUpdate = 'https://dreamclever.ru/updates/dka_update.xml';
...
public function index() {
  ...
  $data['dkaModuleUpdate'] = $this->linkCreate( 'extension/module/dka_cart/check_dka_update', true );

  // LOAD LANGUAGE
	$this->language->load('extension/module/dka_cart');
  // TITLE PAGE
  ...
  // EDIT SETTINGS
	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
		...
		$this->response->redirect($this->linkCreate( 'extension/module/dka_cart', true ));
	}

	// LOAD DATA
	$data['urlCartLoadData'] =$this->linkCreate( 'extension/module/dka_cart/dkaCartLoadData', true );
	// DELETE DATA
	$data['urlCartDelete'] = $this->linkCreate( 'extension/module/dka_cart/dkaCartDelete', true );
	// EXPORT EXCEL
	$data['urlExportCart'] = $this->linkCreate( 'extension/module/dka_export_import/dkaExportCart', true );
	// ZIP URL
	$data['filesToZip'] = $this->linkCreate( 'extension/module/dka_export_import/getFilesToZip', true );
	// SEND MAIL EMAIL
	$data['urlMailSend'] = $this->linkCreate( 'extension/module/dka_cart/dkaMailSend', true );
	// STARTUP NO SHOW
	$data['urlNoShow'] = $this->linkCreate( 'extension/module/dka_cart/dkaNoShow', true );
	$data['urlProductAnalytics'] = $this->linkCreate( 'extension/module/dka_cart/dkaNoShow', true );
  ...
  // FOOTER
	...
	$data['dka_cart_startup_noshow']  = $this->config_settigs('dka_cart_startup_noshow');
	$data['dka_cart_update']  = $this->config_settigs('dka_cart_update');
	$data['dka_cart_update_modal']  = $this->config_settigs('dka_cart_update_modal');

  // BREADCRUMBS
	$data['breadcrumbs'] = array();
	$data['breadcrumbs'][] = $this->returnBread( 'text_home', 'common/dashboard' );
	$data['breadcrumbs'][] = $this->returnBread( 'text_module', 'extension/module' );
	$data['breadcrumbs'][] = $this->returnBread( 'heading_title', 'extension/module/dka_cart' );

	$data['action'] = $this->linkCreate( 'extension/module/dka_cart', true );

  $data['header'] = $this->load->controller('common/header');
  $data['column_left'] = $this->load->controller('common/column_left');
  $data['footer'] = $this->load->controller('common/footer');

  $this->response->setOutput($this->load->view('extension/module/dka_cart.tpl', $data));
}
/* ------------------------  VERSION COMPARE   ------------------------------- */
public function version_compare( $symbol, $compare ){
	$version_full = explode('.', VERSION);
	$version_full = (string)$version_full[0] . (string)$version_full[1] . (string)$version_full[2] . (string)$version_full[3];
	$result = null;
	switch ($symbol) {
		case '>':
			 (int)$version_full > (int)$compare ? $result = true : $result = false;
			break;
		case '>=':
			(int)$version_full >= (int)$compare ? $result = true : $result = false;
			break;
...
/* ------------------------  HEADER RETURN   ------------------------------- */
protected function returnHeader( $data, $type ){
	switch ($type) {
		case 'json':
			$this->response->addHeader('Content-Type: application/json; Charset=UTF-8');
			$this->response->setOutput(json_encode($data));
...
/* ------------------------  CONFIG SETTINGS GET   ------------------------------- */
protected function config_settigs( $setting_params ){
	isset($this->request->post[$setting_params]) ?	$result = $this->request->post[$setting_params] :	$result = $this->config->get($setting_params);
	return $result;
}
/* ------------------------  LINK AJAX   ------------------------------- */
protected function linkCreate( $route, $token, $addition_params = '' ){
	$ssl_version = null;
	if( $this->version_compare( '<', 2200 ) ){ $ssl_version = 'SSL'; } else { $ssl_version = true; }

	$token == true
	? $result = $this->url->link($route, 'token=' . $this->session->data['token'] . $addition_params, $ssl_version)
	: $result = $this->url->link($route, '', $ssl_version);

	return $result;
}

/* ------------------------  BREADCRUMBS   ------------------------------- */
protected function returnBread( $bread_text, $bread_link ){
	$result = array(
		'text' => $this->language->get($bread_text),
		'href' => $this->linkCreate( $bread_link, true )
	);
	return $result;
}
...
protected function validate() {
	if (!$this->user->hasPermission('modify', 'extension/module/dka_cart')) {
...
/* ------------------------  CURL   ------------------------------- */
public function dkaCurl($url,$headers) {
		$handle=curl_init();
...
//  if (isset($this->request->get['dkaModuleAddressUpdate'])) {
		...
		$dka_name = $this->dkaModuleNameUID;
		$dka_version = $this->dkaModuleMajorVersion . "." . $this->dkaModuleMinorVersion . "." . $this->dkaModuleBuildVersion;
		$dka_opencart = $this->dkaOpencartVersion;

    $result = $this->dkaCurl($this->dkaModuleAddressUpdate . "?dka_name=" . $dka_name . "&dka_version=" . $dka_version . "&dka_opencart=" . $dka_opencart, array());
		$xml = null;
		try{
			$xml=new SimpleXMLElement($result['response']);
		}catch(Exception $e){
			$this->returnHeader($e->getMessage(), 'json');
		}
		if($xml){
...
public function dkaCartLoadData(){
	...
	$this->load->model('extension/dka/dka_cart');
	strlen($string) > 0
	? $data = $this->model_extension_dka_dka_cart->dkaGetSearchCarts($start, $limit, $string)
	: $data = $this->model_extension_dka_dka_cart->dkaGetCarts($start, $limit) ;
...
public function dkaCartDelete(){
	...
	$this->load->model('extension/dka/dka_cart');
	$clear_is == false
	? $data = $this->model_extension_dka_dka_cart->dkaCartDelete($cart_id)
	: $data = $this->model_extension_dka_dka_cart->dkaCartsDelete() ;
...
public function dkaMailSend(){
	$this->load->language('extension/module/dka_cart');
